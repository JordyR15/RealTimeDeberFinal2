...
import com.example.realtimedeber.ml.CreateModel;
import com.example.realtimedeber.ml.CreateModelMascotas;
...
public class MainActivity extends AppCompatActivity implements OnSuccessListener<Text>,
    ...
    public void PersonalizedModel(View v) {
        try {
            CreateModel model = CreateModel.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(mSelectedImage);

            CreateModel.Outputs outputs = model.process(image);
    ...
    private void processImage() {
        ...
        try {
            CreateModelMascotas model = CreateModelMascotas.newInstance(getApplicationContext());
            TensorImage image = TensorImage.fromBitmap(rgbFrameBitmap);

            CreateModelMascotas.Outputs outputs = model.process(image);
            List<Category> probability = outputs.getProbabilityAsCategoryList();
            Collections.sort(probability, new CategoryComparator());
            String res="";
            for (int i = 0; i < probability.size(); i++)
